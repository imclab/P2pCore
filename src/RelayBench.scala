...
object RelayBench {
  def main(args:Array[String]): Unit = {
    new RelayBench().start
  }
}

class RelayBench extends RelayTrait {
  ...
  override def receiveMsgHandler(str:String) {
    if(str=="data") {
      ...
      // todo: take end time (incomingDataCounter must be 5000)
